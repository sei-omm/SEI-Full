    ...
    const { rows } = await pool.query(
      `
       SELECT
          ...
          GREATEST(cb.batch_fee - SUM(p.paid_amount) - (SELECT SUM(discount_amount) FROM payments WHERE form_id = ebc.form_id), 0) AS total_due,
...
export const getReceiptReport = asyncErrorHandler(
  async (req: Request, res: Response) => {
    ...
    const { rows } = await pool.query(
      ...
      -- GREATEST(cb.batch_fee - SUM(p.paid_amount) - (SELECT SUM(discount_amount) FROM payments WHERE form_id = p.form_id), 0) AS total_due,
      s.indos_number,
      s.mobile_number,
      s.email,
      p.payment_type,
