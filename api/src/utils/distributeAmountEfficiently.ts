...
export function distributeAmountEfficiently(
  ...
  for (const batch of batches) {
    if (batch[value] > 0) {
      const deduction = Math.min(perBatchShare, batch[value]);
      const distributedAmount = amount > 0 ? deduction : -deduction; // Handle positive or negative amount
      distributionResult.push({
        batch_id: batch.batch_id,
        course_id: batch.course_id,
        distributedAmount,
