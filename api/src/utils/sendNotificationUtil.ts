...
export const sendNotificationUtil = async ({
  ...
  try {
    ...
    if (notification_type === "private" && employee_ids !== undefined) {
      await dbClient.query(
        `
          INSERT INTO notification_sended
            (notification_id, employee_id)
          VALUES
          ${sqlPlaceholderCreator(2, employee_ids.length).placeholder}
              `,
      ...
      await dbClient.query(
        `
          ...
          VALUES
          ${sqlPlaceholderCreator(2, employee_roles.length).placeholder}
              `,
