...
export default function CellTimeTable({
  ...
  const checkDuplicateFac = () => {
    ...
    const virtual_table_array = Object.entries(virtual_table || {}).filter(item => item[1].fac !== null && item[1].fac.faculty_id !== -1);
  ...
  const handleFacChange = (fac_id: number) => {
    ...
    setSingleFacInfo(singleFacInfo);
    if (!virtual_table) return alert("No Row Data Found");

    virtual_table[`${row}:${col}`] = {
      fac: singleFacInfo,
      subject: value || "Off Period",
      ...
      <div className="flex flex-col">
        ...
        <>
          <select
            ...
            name="fac_id"
            defaultValue={singleFacInfo?.faculty_id || "No Faculties"}
            className="px-2 py-1 outline-none cursor-pointer bg-transparent"
          >
